"use strict"
$("#btnListarResFecha").click(aceptarListadoFecha);

function aceptarListadoFecha(){
    let datosEntrada=frmListadoResFecha.txtEntradaResFecha.value;
    let datosSalida=frmListadoResFecha.txtSalidaResFecha.value;
        var oFechas = {
          fechaEntrada: datosEntrada,
          fechaSalida:  datosSalida
        };
        var sParametros = "datos=" + JSON.stringify(oFechas);
        $.get("Reserva/listadoReservaFecha.php", sParametros, repuestasReservaFecha, 'json');


}
function repuestasReservaFecha(oDatos, sStatus, oXHR){
