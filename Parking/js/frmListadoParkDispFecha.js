"use strict"
$("#btnListarParkDispFecha").click(aceptarListadoHabitacionesFecha);

function aceptarListadoHabitacionesFecha(){
    let datosEntrada=frmListadoParkDispFecha.txtEntradaParkDispFecha.value;
    let datosSalida=frmListadoParkDispFecha.txtSalidaParkDispFecha.value;
        var oFechas = {
          fechaEntrada: datosEntrada,
          fechaSalida:  datosSalida
        };
        var sParametros = "datos=" + JSON.stringify(oFechas);
        $.get("Parking/listadoParkingFecha.php", sParametros, respuestaParkingFecha, 'json');


}
function respuestaParkingFecha(oDatos, sStatus, oXHR){
